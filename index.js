...
function addBaseOutput() {
    ...
    */
	...
	output.push({
		...
		subtitle: `rates last updated: ${lastUpdate.toLocaleString(
			locale
		)}, next update: ${nextUpdate.toLocaleString(locale)}`,
		arg: "1",
		autocomplete: "1",
		icon: {
	...
	if (currency !== baseCurrency) {
		...
		output.push({
			title: `${amount.toLocaleString(
				locale
			)} ${currency} = ${baseAmount.toLocaleString(locale)} ${baseCurrency}`,
			icon: {
				path: `flags/${currency}.png`,
			},
			...
			text: {
				...
				largetype: `${amount.toLocaleString(
					locale
				)} ${currency} = ${baseAmount.toLocaleString(locale)} ${baseCurrency}`,
			},
		});
		...
		output.push({
			title: `${amount.toLocaleString(
				locale
			)} ${baseCurrency} = ${currencyAmount.toLocaleString(
				locale
			)} ${currency}`,
			icon: {
				path: `flags/${baseCurrency}.png`,
			...
			text: {
				...
				largetype: `${amount.toLocaleString(
					locale
				)} ${baseCurrency} = ${baseAmount.toLocaleString(locale)} ${currency}`,
			},
		});
	}
}
...
function addCurrencyOutput(multiplier, currency) {
	...
	output.push({
		...
		subtitle: `${amount.toLocaleString(
			locale
		)} ${baseCurrency} = ${currencyAmount.toLocaleString(locale)} ${currency}`,
		icon: {
			path: `flags/${currency}.png`,
		},
...
function updateList(list) {
	if (favoriteCurrency == "" || typeof favoriteCurrency == "undefined") {
...
if (nextUpdate < new Date()) {
	...
	output.push({
		title: `INFO: Rates for ${baseCurrency} updated: ${new Date(
			alfy.cache.get("updateDate")
		).toLocaleString(locale)}`,
		subtitle: `next update: ${new Date(
			alfy.cache.get("nextUpdate")
		).toLocaleString(locale)}`,
		icon: {
			path: "settings.png",
		},
...
if (query[0] !== "BASE" && (!baseCurrency || !lastUpdate)) {
	...
	if (!query[0]) {
		...
		currencies
			.filter((currency) => currency.includes(query[0]))
			.forEach((currency) => addSetBaseCurrencyListOutput(currency));
    ...
    before all
*/
	(favoriteCurrency == "" || typeof favoriteCurrency == "undefined") &&
	query[0] !== "LIST" &&
	query[0] !== "BASE"
) {
	promises.push(getOSLocale());
	if (!query[0]) {
		...
		currencies
			.filter((currency) => currency.includes(query[0]))
			.forEach((currency) => addfavoriteCurrencyListOutput(currency));
	} else if (currencies.includes(query[0])) {
		addfavoriteCurrencyListOutput(query[0]);
	}
} else if (!query[0]) {
	favoriteCurrency.forEach((currency) =>
		addCurrencyMainListOutput("1", currency)
	);
	...
	if (query[1].match("^[A-Z]{1,2}$")) {
		currencies
			.filter((currency) => currency.includes(query[1]))
			.forEach((currency) => addSetBaseCurrencyListOutput(currency));
...
*/
	...
	if (typeof favoriteCurrency != "undefined" && favoriteCurrency != "") {
		// only if there's a value in list
		favoriteCurrency.forEach((currency) =>
			removeFavoriteCurrencyListOutput(currency)
		);
	}
} else if (query[0] === "RMVE" && query[1]) {
	if (typeof favoriteCurrency != "undefined" && favoriteCurrency != "") {
		if (query[1].match("^[A-Z]{1,2}$")) {
			favoriteCurrency
				.filter((currency) => currency.includes(query[1]))
				.forEach((currency) => removeFavoriteCurrencyListOutput(currency));
	...
	if (query[1].match("^[A-Z]{1,2}$")) {
		currencies
			.filter((currency) => currency.includes(query[1]))
			.forEach((currency) => addCurrencyListOutput(query[0], currency));
